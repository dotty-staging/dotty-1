...
object Json:
  final case class Obj(value: Map[String, Json]) extends Json, scala.Dynamic:
    def selectDynamic(name: String): Json | Undefined.type = value.getOrElse(name, Undefined)
  ...
  given ToExpr[Json] with
    def apply(x: Json)(using Quotes): Expr[Json] =
      x match
        ...
        case Json.Str(value) => '{ Json.Str(${Expr(value)}) }
