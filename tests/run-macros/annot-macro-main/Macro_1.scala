    ...
    tree match
      case DefDef(name, List(TermParamClause(Nil)), _, _) =>
        ...
        def decls(cls: Symbol): List[Symbol] =
          List(Symbol.newMethod(cls, "main", MethodType(Nil)(_ => Nil, _ => TypeRepr.of[Unit]), Flags.Static, Symbol.noSymbol))

        val cls = Symbol.newClass(tree.symbol.owner.owner, name, parents = parents.map(_.tpe), decls, selfType = None)
