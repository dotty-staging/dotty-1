      ...
      scalaCompilerBridgeBinaryJar := Def.settingDyn {
        if (isDotty.value) Def.task {
          val updateReport = fetchArtifactsOf(
            scalaOrganization.value % "dotty-sbt-bridge" % (scalaVersion.value + "-zinc-preview"),
          ...
          Option(getJar(updateReport, scalaOrganization.value, "dotty-sbt-bridge", scalaVersion.value + "-zinc-preview"))
        }
        else Def.task {
          None: Option[File]
        }
