...
class CodeGen(val int: DottyBackendInterface, val primitives: DottyPrimitives)( val bTypes: BTypesFromSymbols[int.type]) { self =>
  ...
  private def onFileCreated(cls: ClassNode, claszSymbol: Symbol, sourceFile: dotty.tools.dotc.util.SourceFile): AbstractFile => Unit = clsFile => {
    ...
    if (ctx.sbtCallback != null) {
      import dotty.tools.dotc.sbt.virtualFile
      val jSourceFile = sourceFile.file.virtualFile
