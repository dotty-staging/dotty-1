...
class ClassfileWriterOps(outputDir: AbstractFile)(using @constructorOnly ictx: Context) {
  ...
  def writeClass(className: InternalName, bytes: Array[Byte], sourceFile: AbstractFile)(using Context): AbstractFile | Null = try {
    ...
    case e: FileConflictException =>
      report.error(em"error writing $className: ${e.getMessage}")
      null
    case e: java.nio.file.FileSystemException =>
      if ctx.settings.Ydebug.value then e.printStackTrace()
      report.error(em"error writing $className: ${e.getClass.getName} ${e.getMessage}")
      null
  }

  def writeTastySig(className: InternalName, bytes: Array[Byte], isJava: Boolean): Unit =
    val suffix = if isJava then ".jsigtasty" else ".sigtasty"
    writeToJarOrFile(className, bytes, suffix) // extra 's' to differentiate from tasty - simple way to avoid opening too many files
