...
object report:
  ...
  /** Report an error at the position of the macro expansion and throws a internal.StopMacroExpansion */
  def throwError(msg: => String)(using qctx: QuoteContext): Nothing = {
    error(msg)
    throw new internal.StopMacroExpansion
  }
  /** Report an error at the on the position of `expr` and throws a internal.StopMacroExpansion */
